...
import { House, LogOut, Bell, ChevronDown, User, Shield, Menu, X } from 'lucide-react'
...
const Navbar = () => {
  ...
  return (
    <>
      <header className="sticky top-0 z-50 border-b border-slate-200 bg-white/95 backdrop-blur-sm shadow-sm shadow-slate-100">
        <div className="mx-auto flex h-16 w-full max-w-[1400px] items-center justify-between px-6 lg:px-8">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            {/* User menu (Desktop dropdown, hides label on mobile) */}
            <div className="relative">
              <button
                type="button"
                onClick={() => {
                ...
                className="flex items-center gap-2.5 rounded-xl border border-slate-200 bg-white px-2.5 py-2 transition hover:bg-slate-50"
              >
                {/* Avatar */}
                <span className="flex h-7 w-7 items-center justify-center rounded-lg bg-gradient-to-br from-indigo-500 to-violet-500 text-[11px] font-bold text-white shrink-0">
                  {initials}
                </span>
                ...
                <ChevronDown className={`hidden md:block h-3.5 w-3.5 text-slate-400 transition-transform ${menuOpen ? 'rotate-180' : ''}`} />
              </button>

              {menuOpen && (
                <div className="absolute right-0 top-full mt-2 w-52 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-xl shadow-slate-200/80 z-50">
            ...
            {user && (
              <button
                ...
                className="flex h-9 w-9 lg:hidden items-center justify-center rounded-xl border border-slate-200 bg-white text-slate-500 transition hover:bg-slate-50 hover:text-slate-700 cursor-pointer"
              >
      ...
      {user && (
        <div
          className={`fixed inset-y-0 left-0 z-50 w-72 bg-white border-r border-slate-200 shadow-2xl transition-transform duration-300 ease-in-out lg:hidden flex flex-col ${
        ...
        >
          ...
          <div className="flex items-center justify-between border-b border-slate-100 px-5 py-4 shrink-0">
            ...
            <button
              type="button"
              onClick={() => setMobileMenuOpen(false)}
              className="rounded-xl p-2 text-slate-500 hover:bg-slate-100 hover:text-slate-700 transition cursor-pointer"
            >
          ...
          <div className="flex-1 overflow-y-auto p-5 space-y-5 text-left flex flex-col justify-between">
            <div className="space-y-5">
              {/* User profile brief */}
              <div className="flex items-center gap-3 pb-4 border-b border-slate-100">
                <span className="flex h-9 w-9 items-center justify-center rounded-xl bg-gradient-to-br from-indigo-500 to-violet-500 text-xs font-bold text-white uppercase">
                  {initials}
                </span>
                <div className="min-w-0 flex-1">
                  <p className="truncate text-sm font-semibold text-slate-800 leading-tight">{user?.name}</p>
                  <span className={`mt-0.5 inline-block rounded px-1.5 py-0.5 text-[9px] font-bold uppercase tracking-wider ${roleColor}`}>
                    {user?.role}
                  </span>
                </div>
              </div>
              ...
              <nav className="flex flex-col gap-1">
                <NavLink
                  ...
                  onClick={() => setMobileMenuOpen(false)}
                  className={({ isActive }) =>
                    `flex items-center gap-3 rounded-xl px-3.5 py-2.5 text-sm font-semibold transition ${isActive ? 'bg-indigo-50 text-indigo-700' : 'text-slate-600 hover:bg-slate-50'}`
                  }
                >
                  Dashboard
                </NavLink>
            ...
            <div className="pt-4 border-t border-slate-100">
              <button
                ...
                className="flex w-full items-center gap-3 rounded-xl px-3.5 py-2.5 text-sm font-bold text-rose-600 transition hover:bg-rose-50 cursor-pointer"
              >
                <LogOut className="h-4 w-4" />
                Sign Out
              </button>
